...
```{r}
penguin <- read_csv("https://raw.githubusercontent.com/allisonhorst/palmerpenguins/main/inst/extdata/penguins.csv")
...
```{r}
penguin |> 
  mutate(bill_length_centimenter = bill_length_mm/10,
           bill_depth_centimenter = bill_depth_mm/10, 
           flipper_length_centimenter = flipper_length_mm/10)
...
```{r}
  ...
  ifelse(penguin$bill_depth_mm >= 18.0, "high", "low")
...
grouped_by_island <- group_by(penguin, island)
 
summarise(grouped_by_island, body_mass_avg = mean(body_mass_g, na.rm = T))
...
```{r}
new_penguins <- penguin |> 
  filter(species %in% c("Chinstrap", "Adelie")) |> 
  rename(observed_sex = sex) |> 
  select(species, observed_sex, bill_depth_mm, bill_length_mm) |>
...
penguin |> 
  ...
  summarise(ratio = mean(body_mass_g, na.rm = T))
...
```{r}
animal_tidy <- animal_friends |> 
  separate(col = Age.Adopted.Condition, sep = "/", into = c("Age", "Year_Adopted", "Temperament"))
...
animal_tidy <- animal_tidy |> 
  rename(adopted = a_opterr) |> 
  remove_constant()
...
animal_tidy |> 
  pivot_longer(c(age, year_adopted, temperament))
